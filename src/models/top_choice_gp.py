...
from src.models.likelihoods.top_choice import (
    TopChoiceLikelihood,
    TopChoiceLogitLikelihood,
)


class TopChoiceGP(Model, GP):
    r"""Top choice GP for preference learning with Laplace approximation

    Example:
        >>> from botorch.models import TopChoiceGP
        >>> import torch
        >>> datapoints = torch.Tensor([[1, 2, 3], [4, 5, 6], [7, 8, 9]])
        >>> choices = torch.Tensor([[0, 1], [2, 1]])
        >>> model = PairwiseGP(datapoints, choices)
    """
    ...
    def __init__(
        ...
        datapoints: Tensor,
        choices: Tensor,
        likelihood: Optional[TopChoiceLikelihood] = None,
        ...
        **kwargs,
        ...
        Args:
            ...
            choices: A `batch_shape x m x 2` training choices;
                choices[i] is a noisy indicator suggesting the utility value
                of choices[i, 0]-th is greater than choices[i, 1]-th.
        ...
        self.likelihood = (
            TopChoiceLogitLikelihood(k_choice=choices.shape[-1])
        ...
        self.set_train_data(datapoints, choices, update_model=False)
        ...
        if self.datapoints is not None and self.choices is not None:
    ...
    def __deepcopy__(self, memo) -> TopChoiceGP:
        attrs = (
            "datapoints",
            "choices",
            "covar",
    ...
    def set_train_data(
        ...
        r"""Set datapoints and choices and update model properties if needed
        ...
        Args:
            ...
            choices: A tensor of size `batch_shape x m x 2`. (i, j) means
            ...
            update_model: True if we want to refit the model (see _update) after
                re-setting the data.
        """
        # When datapoints and/or choices are None, we are constructing
        # a prior-only model
        if datapoints is None or choices is None:
            return
        ...
        if choices is not None:
            if strict:
                self._check_strict_input([choices], [self.train_targets], "targets")

            # convert to long so that it can be used as index and
            # compatible with Tensor.scatter_
            self.choices = choices.long()
            # Compatibility variables with fit_gpytorch_*
            # alias for choices ("train_targets" here)
            self.train_targets = self.choices
        ...
        self.m = self.choices.shape[-2]  # num pairwise choices
        self.k_choice = self.choices.shape[-1]
        # D is batch_size x m x n or num_choices x num_datapoints.
        # D_k_i is the s_k(x_i) value as in equation (6) in [Chu2005preference]_
        # D will usually be very sparse as well
        # TODO swap out scatter_ so that choices could be int instead of long
        ...
        choice_view = self.choices.view(-1, self.m, self.k_choice).long()
        for i, sub_D in enumerate(self.D.view(-1, self.m, self.n)):
            sub_D.scatter_(1, choice_view[i, :, [0]], 1)
            sub_D.scatter_(1, choice_view[i, :, 1:], -1)
    ...
    def forward(self, datapoints: Tensor) -> MultivariateNormal:
        ...
        if self.training:
            if self._has_no_data():
                raise RuntimeError(
                    "datapoints and choices cannot be None in training mode. "
            ...
            self.set_train_data(datapoints, self.choices, update_model=True)
    ...
    def condition_on_observations(self, X: Tensor, Y: Tensor, **kwargs: Any) -> Model:
        ...
        if self._has_no_data():
            ...
            # Can only condition on pairwise choices instead of the directly
            ...
            # Reshaping datapoints and choices by batches
            ...
            new_choices = self.choices.expand(
                Y_new_batch_shape + self.choices.shape[-2:]
            )
            ...
            shifted_comp = Y.to(new_choices) + self.n
            new_choices = torch.cat((new_choices, shifted_comp), dim=-2)

            # TODO: be smart about how we can update covar matrix here
            new_model.set_train_data(new_datapoints, new_choices, update_model=True)

        return new_model


class TopChoiceLaplaceMarginalLogLikelihood(MarginalLogLikelihood):
